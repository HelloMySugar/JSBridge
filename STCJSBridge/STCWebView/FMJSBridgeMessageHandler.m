...
#import "STCWKWebViewBridge+Private.h"
...
@interface FMJSBridgeMessageHandler () {
    __weak STCWKWebViewBridge *_webViewBridge;
}
@end
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    NSArray  *args   = body[@"data"];
    
    [self callNative:@"" method:method arguments:args];
...
- (void)callNative:(NSString *)module
    ...
    NSObject <STCWKJavascriptInterfaceDelegate> *interface = _webViewBridge.javascriptInterfaces[module];
    
    STCBridgeMessage *message = interface.message;
    if (message) {
...
- (void)queueMessage:(NSDictionary *)message {
    ...
    NSString *javascriptCommand = [NSString
                                   stringWithFormat:@"javascript:_invokeCallback('%@');",
                                   messageJSON];
